...
export default class IntrepidLoanEngineIntegration {
    ...
    async fetchContext(entityId) {
        ...
        console.log(`[Intrepid Loan Engine Node] Loading tenant-scoped loan processing run context for: ${entityId} (${mode} mode)`);
    ...
    async fetchCubeRunContext(entityId) {
    ...
    async loadSingleCubeRow(cubeUrl, token, query) {
        const response = await axios.post(`${cubeUrl}/cubejs-api/v1/load`, { query }, {
            headers: {
