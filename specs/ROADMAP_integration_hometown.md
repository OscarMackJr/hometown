...
Status: Active
Version: revised 2026-07-10 to integrate the Popeye program work (reporting tier, answer provenance, governed access)
...
hometown does NOT own: the gateway, virtual keys, budgets, provider credentials, gateway infrastructure, endpoint (nono) policy, or the popeye-infra Terraform. If a task involves any of those, it belongs to the popeye team.
...
- [Answer Trace Envelope v0.1](./ANSWER_TRACE_ENVELOPE_v0.1.md) - the Stage 4 provenance contract.
- Popeye program documents (popeye-infra repo, specs/): the governance roadmap v0.2, the infrastructure plan, and the Stage 1 kickoff. Read for context; their tasks are not this team's tasks except where restated below.
...
- Evaluation harness (Popeye program POC C) consumes Stage 4 traces; its golden-question sets are owned per domain, its harness placement is undecided.
